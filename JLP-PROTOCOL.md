...
- Extra fields:
  ...
  - `ticks` (int, optional) — N evenly-spaced major tick marks around the arc. `0` or omitted = none. Drawn just inside the arc ring's inner edge so the ring (painted on top) can't cover them.
  - `tick_labels` (bool, optional, default `false`) — print each tick's display-space value next to it, in the same muted color as the tick itself. Rendered by the firmware (and, via the shared `widget_factory.cpp`, the designer's WASM preview) — there is no separate designer-side rendering path.
...
- **Local action sentinel** `bind: "@brightness"` — instead of a SignalK path, the slider becomes this **panel's backlight control**, 5–100 %. It is the same `cockpit.brightness` setting the config API exposes: dragging previews the level on the backlight at once, releasing persists it, so it survives a reboot and the idle dimmer restores it on the next wake. A change made elsewhere (the config API, the designer) moves the knob. No SignalK PUT and no SignalK subscription. `min`, `max` and `display` are ignored; the range is the setting's. One per layout.
- Subject kind: `Float`.
